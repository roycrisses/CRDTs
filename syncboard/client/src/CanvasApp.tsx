...
interface FabricObjectWithId extends fabric.Object {
  ...
  radius?: number;
  text?: string;
  item?: (index: number) => fabric.Object;
...
export const CanvasApp = () => {
  ...
  const [selectedObject, setSelectedObject] = useState<{ id: string; type: string; fill: string; subType?: string } | null>(null);
  ...
  useEffect(() => {
    ...
    if (fabricRef.current) {
      ...
      fabricRef.current.defaultCursor = getCursor();
    ...
    fabricCanvas.on('selection:created', (e) => {
      ...
      if (obj && obj.id) {
        setSelectedObject({
          id: obj.id,
          type: obj.type || '',
          fill: (obj.fill as string) || (obj.item ? (obj.item(0).fill as string) : ''),
          subType: obj.subType
        });
      }
    });
  ...
  const addShape = useCallback((type: Tool) => {
    ...
    const data: ElementData = {
      ...
      subType: type === 'sticky' || type === 'arrow' ? type : undefined,
  ...
  const updateObjectProperty = (properties: { fill?: string; stroke?: string }) => {
    ...
    if (data) {
      yElementsRef.current.set(selectedObject.id, {
        ...data,
        style: { ...data.style, ...properties }
      });
      setSelectedObject({ ...selectedObject, fill: properties.fill || selectedObject.fill });
    }
  };
