...
export type ElementData = {
  ...
  content?: string;
  style: { fill: string; stroke?: string; strokeWidth?: number; angle?: number };
  path?: (string | number)[][];
};
...
export const CanvasApp = () => {
  ...
  const [selectedObject, setSelectedObject] = useState<{ id: string; color: string; position: { x: number; y: number } } | null>(null);
  ...
  useEffect(() => {
    ...
    if (fabricRef.current) {
      ...
      fabricRef.current.defaultCursor = activeTool === 'hand' ? 'grab' : (activeTool === 'select' ? 'default' : 'crosshair');
  ...
  useEffect(() => {
    ...
    fabricCanvas.on('after:render', () => {
      ...
      for (let i = vpt[4] % scaledGrid; i < fabricCanvas.width!; i += scaledGrid) {
        ctx.moveTo(i, 0);
        ctx.lineTo(i, fabricCanvas.height!);
      }
      for (let i = vpt[5] % scaledGrid; i < fabricCanvas.height!; i += scaledGrid) {
        ctx.moveTo(0, i);
      ...
      if (activeObject && activeObject.id) {
        ...
        if (canvasRect) {
          ...
          if (activeObject.fill && typeof activeObject.fill === 'string') {
            ...
            const firstItem = activeObject.item(0);
            if (firstItem) {
              color = firstItem.stroke || (firstItem.fill as string) || '#000000';
            }
          }
          ...
          setSelectedObject({
            ...
            color,
            position: {
              x: canvasRect.left + boundingRect.left + boundingRect.width / 2,
              y: canvasRect.top + boundingRect.top
            }
          });
        }
      } else {
    ...
    fabricCanvas.on('selection:created', updatePropertyMenu);
    fabricCanvas.on('selection:updated', updatePropertyMenu);
    fabricCanvas.on('selection:cleared', () => setSelectedObject(null));
    fabricCanvas.on('object:moving', updatePropertyMenu);
    fabricCanvas.on('object:scaling', updatePropertyMenu);
    ...
    fabricCanvas.on('mouse:wheel', (opt) => {
      ...
      fabricCanvas.zoomToPoint({ x: opt.e.offsetX, y: opt.e.offsetY }, zoomLevel);
      setZoom(zoomLevel);
      updatePropertyMenu();
