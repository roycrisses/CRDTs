...
import { MousePointer2, Square, Circle as CircleIcon, Type, StickyNote, Pencil, Trash2, Hand, ArrowRight, Highlighter, Image as ImageIcon } from 'lucide-react';
...
export const Toolbar: React.FC<ToolbarProps> = ({ activeTool, setActiveTool, onClear, onImageUpload }) => {
  ...
  return (
    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-1 p-1.5 bg-white/90 backdrop-blur-2xl rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-slate-200/50 z-50 transition-all duration-300">
      <input
        type="file"
        ref={fileInputRef}
        className="hidden"
      ...
      <button
        onClick={onClear}
        className="p-2.5 rounded-xl text-slate-400 hover:bg-rose-50 hover:text-rose-500 transition-all duration-200 group relative"
        title="Clear Board"
      >
        <Trash2 size={20} />
