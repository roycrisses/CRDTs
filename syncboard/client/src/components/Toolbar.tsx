...
export const Toolbar: React.FC<ToolbarProps> = ({ activeTool, setActiveTool, onClear, snapToGrid, setSnapToGrid }) => {
  ...
  return (
    <div className="fixed left-6 top-1/2 -translate-y-1/2 flex flex-col gap-2 p-2 bg-white/90 backdrop-blur-2xl rounded-2xl shadow-2xl border border-slate-200/50 z-50">
      {tools.map((tool) => (
        <button
          key={tool.id}
      ...
      <button
        ...
        className={cn(
          "p-3 rounded-xl transition-all duration-200 group relative",
          snapToGrid ? "bg-indigo-50 text-indigo-600" : "text-slate-500 hover:bg-white"
        )}
        title="Snap to Grid"
      >
        <Grid3X3 size={22} />
