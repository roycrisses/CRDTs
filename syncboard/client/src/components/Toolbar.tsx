    ...
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-1 p-1.5 bg-white/90 backdrop-blur-2xl rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-slate-200/50 z-50">
      <div className="flex items-center gap-1 px-1">
        {tools.map((tool) => (
          ...
          >
            ...
            <span className="absolute bottom-14 left-1/2 -translate-x-1/2 px-2 py-1 bg-slate-900 text-white text-[10px] font-bold rounded opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap transition-opacity">
              {tool.label}
            </span>
          </button>
        ))}
      ...
      <div className="flex items-center gap-1 px-1">
        <button
          ...
          className={cn(
            "p-2.5 rounded-xl transition-all duration-200 group relative",
            snapToGrid ? "bg-indigo-50 text-indigo-600" : "text-slate-500 hover:bg-slate-100"
          )}
          title="Snap to Grid"
        >
          <Grid3X3 size={20} />
        ...
        >
          <Trash2 size={20} />
          <span className="absolute bottom-14 left-1/2 -translate-x-1/2 px-2 py-1 bg-rose-600 text-white text-[10px] font-bold rounded opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap transition-opacity">
            Clear Board
          </span>
        </button>
      </div>
