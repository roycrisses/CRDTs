...
export const Toolbar: React.FC<ToolbarProps> = ({ activeTool, setActiveTool, onClear, snapToGrid, setSnapToGrid }) => {
  ...
  return (
    <div className="fixed left-6 top-1/2 -translate-y-1/2 flex flex-col gap-2 p-2 bg-white/90 backdrop-blur-2xl rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-slate-200/50 z-50">
      ...
      <div className="relative group p-3 rounded-xl transition-all duration-200 text-slate-500 hover:bg-white hover:text-slate-900 shadow-sm border border-transparent hover:border-slate-100 cursor-pointer">
        <ImageIcon size={22} />
        <input
          type="file"
          accept="image/*"
      ...
      <button
        ...
        className={cn(
          "p-3 rounded-xl transition-all duration-200 group relative",
          snapToGrid ? "bg-indigo-100 text-indigo-600" : "text-slate-500 hover:bg-white"
        )}
        title="Snap to Grid"
      >
        <Grid size={22} />
